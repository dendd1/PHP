...
echo '<h2>Задание 2</h2>
            <br>';
$input = 'http://asozd.duma.gov.ru/main.nsf/(Spravka)?OpenAgent&RN=366426-7&11';
echo $input . '--------->';
...
$copy_value=$value[0];//копирование значений
foreach($copy_value as &$element) {
    $element = str_replace("/", "\/", $element);
    $element = str_replace("?", "\?", $element);
    $element = str_replace("(", "\(", $element);
    $element = str_replace(")", "\)", $element);
    $element = "/" . $element . "/";
}//преобразование копии в регулярное выражение
foreach ($value[0] as &$element) {
    preg_match("/[0-9]{1,10}[-][0-9]{1,10}/", $element, $number);//поиск номера указа
    $element="http://sozd.parlament.gov.ru/bill/" . $number[0];
}
